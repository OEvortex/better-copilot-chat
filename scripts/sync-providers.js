...
function formatTsObjectKey(key) {
    return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(key) ? key : formatTsString(key);
}
...
function parseKnownProviders() {
    ...
    for (const property of providersNode.properties) {
        ...
        const supportsApiKey = getBooleanProperty(
            providerConfig,
            'supportsApiKey'
        );
    ...
    return buildManagedProviderMetadata(knownProviders)
        .filter(
            (provider) =>
                ...
                provider.supportsApiKey === true
        )
        .map((provider) => ({
            ...provider,
            menuLabel: getAccountUiLabel(provider.id, provider.displayName),
            displayLabel: getAccountDisplayName(
                provider.id,
                    ...
                    properties: {
                        ...Object.fromEntries(
                            Object.entries(
                                ...
                                ([propertyKey]) => propertyKey !== 'baseUrl'
                            )
                        )
                    }
                }
...
function run() {
    ...
    const accountSyncProviderItems =
        buildAccountSyncProviderItems(knownProviders);
