...
import { AccountManager, AccountQuotaCache, AccountSyncAdapter, registerAccountCommands } from './accounts';
...
import { registerCopilotOverviewCommand, registerSettingsPageCommand } from './ui';
import {
    ApiKeyManager,
    CompletionLogger,
    ConfigManager,
    ...
    const registrationPromises = providerEntries.map(async ([providerKey, providerConfig]) => {
        try {
            ...
            if (providerKey === 'zhipu') {
                ...
                try {
                    const chatParticipantResult = await GeminiCliChatParticipant.createAndActivate(context);
                    disposables.push(...chatParticipantResult.disposables);
                    Logger.trace('Gemini CLI chat participant registered with ACP');
                } catch (error) {
                    Logger.warn('Failed to register Gemini CLI chat participant:', error);
                }
