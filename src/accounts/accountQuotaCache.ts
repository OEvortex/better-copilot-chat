...
export class AccountQuotaCache {
	...
	private _onQuotaStateChange =
		new vscode.EventEmitter<QuotaStateChangeEvent>();
	...
	private loadFromStorage(): void {
		...
		try {
			const data =
				this.context.globalState.get<QuotaCacheStorageData>(STORAGE_KEY);
	...
	async markQuotaExceeded(
		...
		state.lastError =
			options?.error ||
			`Quota exceeded, retry after ${Math.round(cooldown / 1000)}s`;
		...
		Logger.debug(
			`[AccountQuotaCache] Account ${accountId} quota exceeded, cooldown ${Math.round(cooldown / 1000)}s (level ${newLevel})`,
		);
	}
	...
	private calculateCooldown(
		...
		let cooldown = QUOTA_BACKOFF_BASE_MS * 2 ** prevLevel;
