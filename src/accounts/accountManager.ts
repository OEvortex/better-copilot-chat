...
export class AccountManager {
    ...
    private async loadFromStorage(): Promise<void> {
        try {
            ...
            if (data && data.version === STORAGE_VERSION) {
                ...
                Logger.debug(
                    `Loaded ${this.accounts.size} accounts from storage`
                );
            }
        } catch (error) {
    ...
    private syncIsDefaultWithActiveAccounts(): void {
        ...
        for (const [provider, accountId] of Object.entries(
            this.activeAccounts
        )) {
    ...
    async addApiKeyAccount(
        ...
        try {
            ...
            Logger.info(
                `Added API Key account: ${displayName} for ${provider}`
            );
    ...
    async removeAccount(accountId: string): Promise<boolean> {
        ...
        try {
            ...
            if (this.activeAccounts[account.provider] === accountId) {
                const remainingAccounts = this.getAccountsByProvider(
                    account.provider
                );
                if (remainingAccounts.length > 0) {
                    this.activeAccounts[account.provider] =
     ...
     */
    private ensureProviderRoutingConfig(
        provider: string
    ): ProviderRoutingConfig {
        ...
        if (
            typeof this.routingConfig[provider].loadBalanceEnabled !== 'boolean'
        ) {
            this.routingConfig[provider].loadBalanceEnabled =
                defaultLoadBalance;
        }
    ...
    async markAccountError(accountId: string, error?: string): Promise<void> {
        await this.updateAccount(accountId, {
            status: 'error',
            metadata: {
                ...this.accounts.get(accountId)?.metadata,
                lastError: error
            }
        });
    }
    ...
    getNextAvailableAccount(
        ...
        if (currentAccountId) {
            ...
            if (
                currentIndex >= 0 &&
                currentIndex < availableAccounts.length - 1
            ) {
