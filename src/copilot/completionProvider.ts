	...
	implements vscode.InlineCompletionItemProvider, vscode.Disposable
{
	...
	private initializeProviders(): void {
		...
		CompletionLogger.trace(
			"[InlineCompletionProvider] Lazy load initialization FIM/NES provider",
		);
		...
		try {
			...
			CompletionLogger.info(
				"[InlineCompletionProvider] FIM/NES provider lazy loading complete",
			);
		} catch (error) {
			CompletionLogger.error(
				"[InlineCompletionProvider] Lazy load initialization provider failed:",
				error,
			);
			throw error;
		}
	...
	activate(): void {
		const CompletionLogger = getCompletionLogger();
		CompletionLogger.trace(
			"[InlineCompletionProvider.activate] Activation started",
		);
		...
		try {
			...
			CompletionLogger.info(
				"[InlineCompletionProvider] Activated (using lazy loading)",
			);
		} catch (error) {
			CompletionLogger.error(
				"[InlineCompletionProvider.activate] Activation failed:",
				error,
			);
			throw error;
		}
						...
						for (const item of nesResult.items) {
							...
							if (
								...
								item.range.end.character ===
									document.lineAt(item.range.end.line).text.length &&
