...
export class AnthropicHandler {
    ...
    private async executeAnthropicRequest(
        ...
        try {
            const client = await this.createAnthropicClient(
                modelConfig,
                accountId
            );
            ...
            Logger.info(
                `[${model.name}] Anthropic request completed`,
                result.usage
            );
        } catch (error) {
            ...
            if (error instanceof Error) {
                if (error.message.includes('401')) {
                    ...
                    errorMessage +=
                        ': Request rate limit, please try again later';
    ...
    private async handleAnthropicStream(
        ...
        let _hasOutputContent = false;
        // Mark whether thinking content was output
        let _hasThinkingContent = false;
        ...
        const flushTextBuffer = (force: boolean) => {
            ...
            if (
                ...
                progress.report(
                    new vscode.LanguageModelTextPart(pendingTextBuffer)
                );
                pendingTextBuffer = '';
                ...
                switch (chunk.type) {
                    ...
                    case 'content_block_start':
                        ...
                        if (chunk.content_block.type === 'tool_use') {
                            ...
                            Logger.trace(
                                `Tool call start: ${chunk.content_block.name}`
                            );
                            ...
                            Logger.trace(
                                'Thinking block start (streaming output)'
                            );
                    ...
                    case 'content_block_stop':
                        ...
                        if (pendingToolCall) {
                            try {
                                ...
                                Logger.debug(
                                    `Tool call complete: ${pendingToolCall.name}`
                                );
                            } catch (e) {
                                Logger.error(
                    ...
                    case 'message_delta':
                        ...
                        if (chunk.delta.stop_reason) {
                            Logger.trace(
                                `Message stop reason: ${chunk.delta.stop_reason}`
                            );
                        }
                        break;
