...
export function convertTools(options: vscode.ProvideLanguageModelChatResponseOptions & { tools?: readonly vscode.LanguageModelChatTool[]; toolMode?: vscode.LanguageModelChatToolMode; modelOptions?: Record<string, unknown> }): {
    ...
    const toolDefs: OpenAIFunctionToolDef[] = tools
        ...
        .map((t: vscode.LanguageModelChatTool) => {
            const name = sanitizeFunctionName(typeof t.name === 'string' ? t.name : String(t.name ?? 'tool'));
            const description = typeof t.description === 'string' ? t.description : '';
            const params = sanitizeSchema((t as { inputSchema?: unknown }).inputSchema ?? { type: 'object', properties: {} });
            return {
    ...
    if (options.toolMode === vscode.LanguageModelChatToolMode.Required) {
        if (tools.length !== 1) {
            Logger.error('[Hugging Face Model Provider] ToolMode.Required but multiple tools:', tools.length);
...
export function validateTools(tools: readonly vscode.LanguageModelChatTool[]): void {
    for (const tool of tools) {
        if (!tool.name.match(/^[\w-]+$/)) {
            Logger.error('[Hugging Face Model Provider] Invalid tool name detected:', tool.name);
        ...
        if (message.role === vscode.LanguageModelChatMessageRole.Assistant) {
            ...
            while (toolCallIds.size > 0) {
                ...
                if (!nextMessage || nextMessage.role !== vscode.LanguageModelChatMessageRole.User) {
                    Logger.error('[Hugging Face Model Provider] Validation failed: missing tool result for call IDs:', Array.from(toolCallIds));
                    ...
                    if (!isToolResultPart(part)) {
                        const ctorName =
                            (Object.getPrototypeOf(part as object) as { constructor?: { name?: string } } | undefined)?.constructor
                                ?.name ?? typeof part;
                        Logger.error('[Hugging Face Model Provider] Validation failed: expected tool result part, got:', ctorName);
                        throw new Error(errMsg);
                    }
                    const callId = (part as { callId: string }).callId;
