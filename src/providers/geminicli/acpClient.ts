...
export class AcpClient implements acp.Client {
    ...
    private currentResponseCallback:
        | ((chunk: string, type: 'text' | 'thought' | 'tool', metadata?: any) => void)
        | null = null;
    ...
    async initialize(): Promise<void> {
        ...
        try {
            Logger.debug(`[ACP] Starting Gemini CLI: ${this.command} ${this.args.join(' ')}`);
            ...
            this.process.on('exit', (code, signal) => {
                Logger.debug(`[ACP] Gemini CLI process exited: code=${code}, signal=${signal}`);
            ...
            Logger.debug(`[ACP] Initialized with protocol version: ${initResult.protocolVersion}`);
    ...
    async sendPrompt(
        ...
        try {
            ...
            if (!sessionId) {
                ...
                Logger.debug(`[ACP] Created session: ${sessionId} for workspace: ${cwd}`);
            } else {
                Logger.debug(`[ACP] Using existing session: ${sessionId} for workspace: ${cwd}`);
            }
            ...
            Logger.debug(`[ACP] Prompt completed with stop reason: ${promptResult.stopReason}`);
    ...
    async requestPermission(params: acp.RequestPermissionRequest): Promise<acp.RequestPermissionResponse> {
        ...
        Logger.info(`[ACP] Auto-approving tool permission: ${params.toolCall.title}`);
        ...
        switch (update.sessionUpdate) {
            case 'agent_message_chunk':
                if (update.content && 'type' in update.content && update.content.type === 'text') {
                    ...
                    Logger.debug(`[ACP] Agent message chunk: ${text.substring(0, 50)}...`);
                }
                break;
            ...
            case 'tool_call':
                {
                    ...
                    const toolCallId = toolCallUpdate.toolCallId || toolCallUpdate.tool_call_id || '';
                    ...
                    Logger.debug(`[ACP] Tool call: ${toolCall.title} (${toolCall.status})`);
            ...
            case 'tool_call_update':
                {
                    ...
                    Logger.debug(`[ACP] Tool call update: ${toolCallId} - ${updateStatus}`);
