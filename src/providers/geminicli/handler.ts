...
export class GeminiHandler {
        ...
        if (this.quotaManager.isInCooldown(quotaKey)) {
            ...
            if (remaining > QUOTA_COOLDOWN_WAIT_MAX_MS) {
                this.debouncedCacheUpdate(
                    ...
                    () =>
                        this.accountQuotaCache.markQuotaExceeded(effectiveAccountId, 'gemini', {
            ...
            for (let idx = 0; idx < baseUrls.length; idx++) {
                ...
                try {
                    ...
                    if (isPermissionDeniedError(result.status, result.body)) {
                        ...
                        throw new Error(result.body || 'Permission denied on Gemini project.');
                    }
                    ...
                    if (category === ErrorCategory.AuthError) {
                        throw new Error('Authentication failed. Please re-login to Gemini CLI.');
                    }
    ...
    private async getProjectId(accessToken: string, baseUrl?: string): Promise<string> {
    ...
    private async streamRequest(
        ...
        if (modelConfig.sdkMode === 'openai' || modelConfig.sdkMode === 'openai-sse') {
            ...
            await new GeminiStreamProcessor().processStream({ response, modelConfig, progress, token });
        }
        return { success: true };
    }
