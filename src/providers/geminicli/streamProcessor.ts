...
export class GeminiStreamProcessor {
	...
	async processStream(options: ProcessStreamOptions): Promise<void> {
		...
		try {
			while (true) {
				...
				this.chunkCounter++;
				if (
					this.chunkCounter % GeminiStreamProcessor.YIELD_EVERY_N_CHUNKS ===
					0
				) {
	...
	private startActivityReporting(
		...
		this.activityReportInterval = setInterval(() => {
			...
			if (
				...
				GeminiStreamProcessor.ACTIVITY_REPORT_INTERVAL_MS
			) {
	...
	private schedulePendingToolCallsFlush(
		...
		this.toolCallFlushInterval = setTimeout(() => {
			...
			this.toolCallFlushInterval = null;
		}, GeminiStreamProcessor.TOOL_CALL_FLUSH_DELAY_MS) as unknown as ReturnType<
			typeof setInterval
		>;
	}
	...
	private flushTextBufferAdaptive(
		...
		const isHighVelocity =
			this.streamVelocity > GeminiStreamProcessor.HIGH_VELOCITY_THRESHOLD;
