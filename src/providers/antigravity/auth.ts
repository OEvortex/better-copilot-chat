...
async function addAllAntigravityModelsToCompatible(
...
async function _showAntigravityModelsQuickPick(
	models: AntigravityModel[],
): Promise<void> {
	...
	quickPick.placeholder =
		"Select models to add to Compatible Provider (multi-select supported)";
	quickPick.items = items;
	quickPick.canSelectMany = true;
	quickPick.ignoreFocusOut = true;
	const selected = await new Promise<ModelQuickPickItem[] | undefined>(
		(resolve) => {
	...
	if (selected && selected.length > 0) {
		...
		if (addedCount > 0) {
			vscode.window.showInformationMessage(
				`Added ${addedCount} Antigravity model(s) to Compatible Provider`,
			);
		}
	}
}

async function doAntigravityLoginAndSave(
	...
	const existingByEmail = existingAccounts.find(
		(acc: { email?: string }) => acc.email === result.email,
	);
	const credentials = {
	...
	vscode.window.showInformationMessage(
		result.projectId
			? `${message} (Project: ${result.projectId})${modelsInfo}`
			: `${message}${modelsInfo}`,
	);
	if (models.length > 0) {
...
function modelName2Alias(originalName: string): string {
	return originalName.startsWith("models/")
		? originalName.substring(7)
		: originalName;
}
...
export class AntigravityAuth {
	static async getProjectId(): Promise<string> {
		...
		try {
			return (
				(JSON.parse(stored) as { project_id?: string }).project_id || ""
			).trim();
		} catch {
			return "";
		}
	...
	static async ensureProjectId(accessToken?: string): Promise<string> {
		const existing = await AntigravityAuth.getProjectId();
		if (existing) {
			return existing;
		}
		const token = accessToken || (await AntigravityAuth.getAccessToken());
		if (!token) {
			return "";
		}
	...
	static async getAccessToken(): Promise<string | null> {
		...
		try {
			...
			const newToken = await AntigravityAuth.refreshToken(
	...
	static async fetchModels(accessToken: string): Promise<AntigravityModel[]> {
		...
		const hardcodedModels: AntigravityModel[] = antigravityConfig.models.map(
			(m) => ({
				id: m.id,
				name: m.name,
				displayName: m.name,
		...
		for (const hostname of endpoints) {
			quotaMap = await AntigravityAuth.tryFetchQuotaFromEndpoint(
				accessToken,
				hostname,
			);
			if (quotaMap.size > 0) {
	...
	private static async tryFetchQuotaFromEndpoint(
		...
		try {
			const response = await fetch(
				`https://${hostname}/v1internal:fetchAvailableModels`,
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${accessToken}`,
	...
	static async getModels(): Promise<AntigravityModel[]> {
		const token = await AntigravityAuth.getAccessToken();
		return token ? AntigravityAuth.fetchModels(token) : [];
	}
	...
	static async refreshModels(): Promise<AntigravityModel[]> {
		...
		const models = await AntigravityAuth.fetchModels(token);
		const stored = await ApiKeyManager.getApiKey(PROVIDER_KEY);
		if (stored) {
			try {
				const authData = JSON.parse(stored);
