...
export class CodexHandler {
        ...
        try {
            ...
            if (currentAccountId) {
                const currentAccount =
                    accountManager.getAccount(currentAccountId);
                if (currentAccount) {
                    ...
                    await this.quotaCache.markQuotaExceeded(
                        currentAccountId,
                        'codex',
                        {
                            resetDelayMs:
                                (error.resets_at - Date.now() / 1000) * 1000,
    ...
    async handleRequest(
        ...
        const rateLimit = getProviderRateLimit('codex', 'openai');
    ...
    private async executeCodexRequest(
        ...
        if (useVSCodeTools) {
            const vsCodeTools =
                this.convertVSCodeToolsToCodex(availableVSCodeTools);
            ...
            if (!hasManageTodoList) {
                ...
                if (manageTodoListTool) {
                    ...
                    Logger.info(
                        '[codex] Added manage_todo_list tool for task planning'
                    );
                }
            }
            ...
            const req = https.request(
                ...
                (res) => {
                    ...
                    if (
                        ...
                        res.on('end', async () => {
                            Logger.error(
                                `[codex] Error ${res.statusCode}: ${body}`
                            );

                            // Handle 429 usage limit reached error
                            if (res.statusCode === 429) {
                                const usageLimitError =
                                    parseUsageLimitError(body);
                                if (usageLimitError) {
                            ...
                            reject(
                                new Error(
                                    `Codex API error: ${res.statusCode} - ${body}`
                                )
                            );
                        });
                        return;
                                ...
                                try {
                                    ...
                                    switch (eventType) {
                                        ...
                                        case 'response.in_progress':
                                            ...
                                            Logger.debug(
                                                `[codex] Lifecycle event: ${eventType}`
                                            );
                                            break;
                                        ...
                                        case 'error': {
                                            ...
                                            const errorType =
                                                data.error?.type || data.type;
                                            const modelParam =
                                                data.error?.param === 'model' ||
                                                data.param === 'model';
                                            const prefix =
                                                modelParam ||
                    ...
                    res.on('end', () => {
                        ...
                        if (!hasTextResponse && buffer.trim().length > 0) {
                            ...
                            for (const trailingLine of trailingLines) {
                                ...
                                const trailingData = trimmedTrailing
                                    .substring(5)
                                    .trim();
                                if (
                                    !trailingData ||
    ...
    private sanitizeToolSchema(schema: unknown): Record<string, unknown> {
        ...
        try {
            sanitized = JSON.parse(JSON.stringify(schema)) as Record<
                string,
                unknown
            >;
        } catch {
