...
function isIntegerLikePropertyName(propertyName: string | undefined): boolean {
	...
	return (
		integerMarkers.some((m) => lowered.includes(m)) || lowered.endsWith("_id")
	);
}
		...
		if (Array.isArray(branch) && branch.length > 0) {
			...
			for (const b of branch) {
				if (
					b &&
					typeof b === "object" &&
					(b as Record<string, unknown>).type === "string"
				) {
	...
	for (const m of messages) {
		...
		for (const part of m.content ?? []) {
			if (part instanceof vscode.LanguageModelTextPart) {
				...
				const content = collectToolResultText(
		...
		if (
			...
			(role === "system" ||
				role === "user" ||
				(role === "assistant" && !emittedAssistantToolCall))
		) {
...
export function convertTools(
	...
	const toolDefs: OpenAIFunctionToolDef[] = tools
		.filter(
			(t): t is vscode.LanguageModelChatTool => !!t && typeof t === "object",
		)
	...
	if (options.toolMode === vscode.LanguageModelChatToolMode.Required) {
		if (tools.length !== 1) {
			Logger.error(
				"[Kilo AI Model Provider] ToolMode.Required but multiple tools:",
				tools.length,
			);
			throw new Error(
...
export function validateTools(
	...
	for (const tool of tools) {
		if (!tool.name.match(/^[\w-]+$/)) {
			Logger.error(
				"[Kilo AI Model Provider] Invalid tool name detected:",
				tool.name,
			);
			throw new Error(
...
export function validateRequest(
	...
	if (!lastMessage) {
		Logger.error("[Kilo AI Model Provider] No messages in request");
		...
		if (message.role === vscode.LanguageModelChatMessageRole.Assistant) {
			const toolCallIds = new Set(
				message.content
					...
					.map(
						(part) =>
							(part as unknown as vscode.LanguageModelToolCallPart).callId,
					),
			);
			...
			while (toolCallIds.size > 0) {
				...
				if (
					...
					Logger.error(
						"[Kilo AI Model Provider] Validation failed: missing tool result for call IDs:",
				...
				nextMessage.content.forEach((part) => {
					if (!isToolResultPart(part)) {
						const ctorName =
							(
								Object.getPrototypeOf(part as object) as
									| { constructor?: { name?: string } }
									| undefined
							)?.constructor?.name ?? typeof part;
						Logger.error(
							"[Kilo AI Model Provider] Validation failed: expected tool result part, got:",
							ctorName,
						);
						throw new Error(errMsg);
					}
...
function mapRole(
	...
	const ASSISTANT = vscode.LanguageModelChatMessageRole
		.Assistant as unknown as number;
...
function collectToolResultText(pr: {
