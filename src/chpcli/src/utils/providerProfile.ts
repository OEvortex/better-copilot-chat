...
function asTrimmedString(value: string | undefined | null): string | undefined {
...
export function loadProfileFile(options?: ProfileFileLocation): ProfileFile | null {
  ...
  try {
    return parseProfileFileJson(readFileSync(filePath, 'utf8'))
  } catch {
    return null
  }
}

export function loadCopilotPlusPlusProfileFile(
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  const persisted =
    options?.persisted ??
    loadCopilotPlusPlusProfileFile(processEnv) ??
    loadProfileFile()
  if (!persisted) {
    return processEnv
  }
