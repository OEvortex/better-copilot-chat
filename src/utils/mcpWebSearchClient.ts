...
export class MCPWebSearchClient {
    ...
    static async clearCache(apiKey?: string): Promise<void> {
        if (apiKey) {
            ...
            for (const [
                key,
                instance
            ] of MCPWebSearchClient.clientCache.entries()) {
                await instance.cleanup();
                Logger.info(
    ...
    private async ensureConnected(): Promise<void> {
        ...
        if (this.isConnecting && this.connectionPromise) {
            Logger.debug(
                '[MCP WebSearch] Waiting for connection to complete...'
            );
    ...
    async search(params: WebSearchRequest): Promise<ZhipuSearchResult[]> {
        Logger.info(
            `[MCP WebSearch] Starting search: "${params.search_query}"`
        );
        ...
        try {
            ...
            const result = await this.client.callTool({
                ...
                arguments: {
                    ...
                    search_recency_filter:
                        params.search_recency_filter || 'noLimit',
                    content_size: params.content_size || 'medium'
                }
            });

            if (Array.isArray(result.content)) {
                const [{ text }] = result.content as {
                    type: 'text';
                    text: string;
                }[];
            ...
            Logger.debug(
                '[MCP WebSearch] Tool invocation finished: no results'
            );
            return [];
        } catch (error) {
            Logger.error(
