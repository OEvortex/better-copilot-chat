...
export class CompletionLogger {
	...
	static initialize(channelName = "CHP-Completion"): void {
		...
		CompletionLogger.outputChannel = vscode.window.createOutputChannel(
			channelName,
			{ log: true },
		);
	}
	...
	static trace(message: string, ...args: unknown[]): void {
		if (CompletionLogger.outputChannel) {
			CompletionLogger.outputChannel.trace(message, ...args);
		}
	}
	...
	static debug(message: string, ...args: unknown[]): void {
		if (CompletionLogger.outputChannel) {
			CompletionLogger.outputChannel.debug(message, ...args);
		}
	}
	...
	static info(message: string, ...args: unknown[]): void {
		if (CompletionLogger.outputChannel) {
			CompletionLogger.outputChannel.info(message, ...args);
		}
	}
	...
	static warn(message: string, ...args: unknown[]): void {
		if (CompletionLogger.outputChannel) {
			CompletionLogger.outputChannel.warn(message, ...args);
		}
	}
	...
	static error(message: string | Error, ...args: unknown[]): void {
		if (CompletionLogger.outputChannel) {
			CompletionLogger.outputChannel.error(message, ...args);
		}
	}
	...
	static dispose(): void {
		if (CompletionLogger.outputChannel) {
			CompletionLogger.outputChannel.dispose();
		}
	}
}
