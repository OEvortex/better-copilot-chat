...
export class OpenAIStreamProcessor {
	...
	private toolCallsInProgress = new Map<
		number,
	...
	private processTextWithThinkingTags(
		...
		while (i < text.length) {
			if (this.isInsideThinkingTag) {
				...
				if (this.thinkingTagBuffer.endsWith(this.CLOSING_TAG)) {
					...
					const safeLength =
						this.thinkingTagBuffer.length - this.CLOSING_TAG_LEN;
		...
		if (!this.isInsideThinkingTag && this.thinkingTagBuffer.length > 0) {
			...
			if (!possibleTagStart) {
				...
				this.thinkingTagBuffer.length > 0 &&
				this.thinkingTagBuffer.length < 10
			) {
	...
	private reportToolCallProgress(
		...
		const _thinkingId = `tool_prep_${Date.now()}`;
	...
	private reportToolCallArgumentsProgress(
		...
		const now = Date.now();
		if (
			now - this.lastToolCallProgressTime >=
			OpenAIStreamProcessor.TOOL_CALL_PROGRESS_INTERVAL_MS
		) {
	...
	private flushTextBufferIfNeeded(
		...
		if (
			...
			this.textBuffer.length >=
				OpenAIStreamProcessor.TEXT_BUFFER_CHAR_THRESHOLD ||
	...
	private reportActivity(
		...
		const now = Date.now();
		if (
			now - this.lastActivityReportTime >=
			OpenAIStreamProcessor.ACTIVITY_REPORT_INTERVAL_MS
		) {
			progress.report(new vscode.LanguageModelTextPart(""));
