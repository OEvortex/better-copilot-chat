...
import { CliKnownProviders, getProviderSdkMode } from '../../providers/cliKnownProviders.js'
import { getExtensionProviderId, getExtensionProviderSdkMode, isUsingExtensionProvider } from '../../utils/model/providers.js'
...
/**
 * Check if the CLI is using an extension-registered provider.
 */
function isActiveExtensionProvider(): boolean {
  return isUsingExtensionProvider()
}

/**
 * Get the base URL for the active extension provider.
 */
function getExtensionProviderBaseUrl(): string | undefined {
  ...
  const sdkMode = getExtensionProviderSdkMode()
  if (sdkMode === 'anthropic') return knownConfig.anthropic?.baseUrl ?? knownConfig.openai?.baseUrl
  if (sdkMode === 'oai-response') return knownConfig.responses?.baseUrl ?? knownConfig.openai?.baseUrl
  return knownConfig.openai?.baseUrl
}

/**
 * Check if a custom base URL has been explicitly set (not derived from provider registry).
 */
...
export function resolveProviderRequest(options?: {
  ...
  // If using extension provider, resolve through its registry
  if (isActiveExtensionProvider() && !hasCustomBaseUrl()) {
    const providerId = getExtensionProviderId()
    const knownConfig = CliKnownProviders[providerId]
    const sdkMode = getExtensionProviderSdkMode()

    // Build the resolved model string from the provider's model list
    const modelId = options?.model?.trim() || process.env.CHP_MODEL || ''
    const finalModel = modelId || options?.fallbackModel || ''

    const baseUrl = options?.baseUrl || getExtensionProviderBaseUrl() || ''
    if (!baseUrl) {
      // Fall through to default resolution if no URL
    } else if (baseUrl) {
      return {
        transport: sdkMode === 'anthropic' ? 'chat_completions' : 'chat_completions',
