...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass .aetherignore to ripgrep when respected', async () => {
      await fs.writeFile(
        path.join(tempRootDir, '.aetherignore'),
        'ignored.txt\n',
      );
      (runRipgrep as Mock).mockResolvedValue({
        stdout: '',
    ...
    it('should include .aetherignore matches when disabled in config', async () => {
      await fs.writeFile(path.join(tempRootDir, '.aetherignore'), 'kept.txt\n');
  ...
  describe('multi-directory workspace', () => {
    ...
    it('should load .aetherignore from each workspace directory', async () => {
      const secondDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'grep-tool-second-'),
      );
      await fs.writeFile(path.join(secondDir, '.aetherignore'), 'ignored.txt\n');
      await fs.writeFile(
        path.join(tempRootDir, '.aetherignore'),
        'other-ignored.txt\n',
      );
      ...
      expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.aetherignore'));
      expect(ignoreFileArgs).toContain(path.join(secondDir, '.aetherignore'));
