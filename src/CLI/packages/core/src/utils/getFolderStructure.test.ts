...
`.trim();
  ...
  describe('with aetherignore', () => {
    it('should ignore aetherignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.aetherignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectAetherIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.aetherignore'),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
