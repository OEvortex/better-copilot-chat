...
describe('DashScopeOpenAICompatibleProvider', () => {
  ...
  describe('isDashScopeProvider', () => {
    it('should return true for AETHER_OAUTH auth type', () => {
      const config = {
        authType: AuthType.AETHER_OAUTH,
  ...
  describe('buildHeaders', () => {
    it('should build DashScope-specific headers', () => {
      ...
      expect(headers).toEqual({
        'User-Agent': `AetherCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-CacheControl': 'enable',
        'X-DashScope-UserAgent': `AetherCode/1.0.0 (${process.platform}; ${process.arch})`,
        'X-DashScope-AuthType': AuthType.AETHER_OAUTH,
      });
    });
    ...
    it('should merge custom headers with DashScope defaults', () => {
      ...
      expect(headers['User-Agent']).toContain('AetherCode/1.0.0');
      expect(headers['X-DashScope-UserAgent']).toContain('AetherCode/1.0.0');
      expect(headers['X-DashScope-AuthType']).toBe(AuthType.AETHER_OAUTH);
    ...
    it('should handle unknown CLI version', () => {
      ...
      expect(headers['User-Agent']).toBe(
        `AetherCode/unknown (${process.platform}; ${process.arch})`,
      );
  ...
  describe('buildRequest', () => {
    ...
    it('should preserve all original request parameters', () => {
      ...
      expect(result.model).toBe('aether-max');
