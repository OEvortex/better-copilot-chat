...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  it('should return the base prompt when no userMemory is provided', () => {
    ...
    expect(prompt).toContain('You are Aether, an interactive CLI agent'); // Check for core content
  ...
  it('should return the base prompt when userMemory is empty string', () => {
    ...
    const prompt = getCoreSystemPrompt('');
    expect(prompt).not.toContain('---\n\n');
    expect(prompt).toContain('You are Aether, an interactive CLI agent');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('You are Aether, an interactive CLI agent'); // Ensure base prompt follows
  ...
  describe('QWEN_SYSTEM_MD environment variable', () => {
    ...
    it('should read from default path when QWEN_SYSTEM_MD is "true"', () => {
      const defaultPath = path.resolve(path.join(AETHER_CONFIG_DIR, 'system.md'));
...
describe('Model-specific tool call formats', () => {
  ...
  it('should use XML format for aether3-coder model', () => {
    vi.mocked(isGitRepository).mockReturnValue(false);
    const prompt = getCoreSystemPrompt(undefined, 'aether3-coder-7b');
  ...
  it('should preserve model-specific formats with user memory', () => {
    ...
    const prompt = getCoreSystemPrompt(userMemory, 'aether3-coder-14b');
