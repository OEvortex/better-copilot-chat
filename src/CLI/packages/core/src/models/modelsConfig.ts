...
export class ModelsConfig {
  ...
  // One-shot flag for aether-oauth credential caching
  private requireCachedAetherCredentialsOnce: boolean = false;
  ...
  private createStateSnapshotForRollback(): {
    ...
    requireCachedAetherCredentialsOnce: boolean;
    ...
    return {
      ...
      requireCachedAetherCredentialsOnce: this.requireCachedAetherCredentialsOnce,
  ...
  private rollbackToStateSnapshot(
    ...
    this.requireCachedAetherCredentialsOnce =
      snapshot.requireCachedAetherCredentialsOnce;
   ...
   */
  getModel(): string {
    return this._generationConfig.model || DEFAULT_AETHER_MODEL;
  }
  ...
  getAllConfiguredModels(authTypes?: AuthType[]): AvailableModel[] {
    ...
    // Force aether-oauth to the front (if requested / defaulted in).
    const orderedAuthTypes: AuthType[] = [];
    if (uniqueAuthTypes.includes(AuthType.AETHER_OAUTH)) {
      orderedAuthTypes.push(AuthType.AETHER_OAUTH);
    }
  ...
  async setModel(
    ...
    // Special case: aether-oauth model switch - hot update in place
    // coder-model supports vision capabilities and can be hot-updated
    if (
      this.currentAuthType === AuthType.AETHER_OAUTH &&
      newModel === DEFAULT_AETHER_MODEL
    ) {
      ...
      if (this.onModelChange) {
        await this.onModelChange(AuthType.AETHER_OAUTH, false);
      }
      return;
    }
  ...
  async switchModel(
    ...
    if (authType === AuthType.AETHER_OAUTH && options?.requireCachedCredentials) {
      this.requireCachedAetherCredentialsOnce = true;
    }
  ...
  consumeRequireCachedCredentialsFlag(): boolean {
    const value = this.requireCachedAetherCredentialsOnce;
    this.requireCachedAetherCredentialsOnce = false;
    return value;
  }
  ...
  private applyResolvedModelDefaults(model: ResolvedModelConfig): void {
    ...
    // For Aether OAuth, apiKey must always be a placeholder. It will be dynamically
  ...
  private checkRequiresRefresh(previousModelId: string): boolean {
    ...
    // For Aether OAuth, model switches within the same authType can always be hot-updated
  ...
  private getRuntimeModelOption(): AvailableModel | undefined {
    ...
    return {
      ...
      /**
       * `isVision` is for automatic switching of aether-oauth vision model.
