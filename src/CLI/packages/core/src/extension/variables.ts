...
export const EXTENSIONS_DIRECTORY_NAME = path.join(AETHER_DIR, 'extensions');
export const EXTENSIONS_CONFIG_FILENAME = 'aether-extension.json';
export const INSTALL_METADATA_FILENAME = '.aether-extension-install.json';
  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".aether" in markdown files
  ...
  try {
    ...
    for (const file of scriptFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".aether" in shell scripts
        // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
        // Avoid matching URLs, comments, or string literals containing .claude
        ...
        if (finalScriptContent !== content) {
          ...
          debugLogger.debug(
            `Updated transcript format and replaced .claude with .aether in shell script: ${filePath}`,
          );
        }
      } catch (error) {
        debugLogger.warn(
