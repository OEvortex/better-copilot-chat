...
describe('substituteHookVariables', () => {
  ...
  it('should handle multiple event types with hooks', () => {
    const basePath = '/home/user/.aether/extensions/my-extension';
...
describe('performVariableReplacement', () => {
  ...
  it('should replace .claude with .aether in markdown files', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('.aether/loop.local.md');
    expect(result).toContain('rm .aether/loop.local.md');
    expect(result).toContain('$HOME/.aether/cache');
    expect(result).toContain('./.aether/local');
  ...
  it('should replace .claude with .aether in shell scripts', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('~/.aether/cache');
    expect(result).toContain('./.aether/local');
    expect(result).toContain('.aether/config');
    // Note: URLs are also being replaced in current implementation
    expect(result).toContain('https://example.com/.aether/page');
  });
