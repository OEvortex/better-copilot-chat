    ...
    if (argsStr) {
      try {
        ...
        if (Array.isArray(parsedArgs)) {
          ...
          debugLogger.error(
            'AETHER_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
          );
        }
      } catch (e) {
        debugLogger.error(
          'Failed to parse AETHER_CODE_IDE_SERVER_STDIO_ARGS:',
          e,
        );
      }
    }
  ...
  private async getLegacyConnectionConfig(
    ...
    if (this.ideProcessInfo) {
      try {
        const portFile = path.join(
          os.tmpdir(),
          `aether-ide-server-${this.ideProcessInfo.pid}.json`,
        );
        const portFileContents = await fs.promises.readFile(portFile, 'utf8');
        return JSON.parse(portFileContents);
      } catch (_) {
        // For older/newer extension versions, the file name matches the pattern
        // /^aether-ide-server-${pid}-\d+\.json$/. If multiple IDE
    ...
    if (portFromEnv) {
      try {
        const portFile = path.join(
          os.tmpdir(),
          `aether-ide-server-${portFromEnv}.json`,
        );
        const portFileContents = await fs.promises.readFile(portFile, 'utf8');
        return JSON.parse(portFileContents);
      } catch (_) {
