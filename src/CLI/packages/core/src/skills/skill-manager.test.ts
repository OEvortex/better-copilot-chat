...
describe('SkillManager', () => {
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.aether/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.aether/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.aether/skills/test-skill/SKILL.md';
...
`;
  ...
  describe('listSkills', () => {
    beforeEach(() => {
      ...
      const userQwenSkillsDir = path.join('/home/user', '.aether', 'skills');
...
Skill 3 content`);
    ...
    it('should deduplicate same-name skills across provider dirs within a level', async () => {
      // Override readdir to return the same skill name from both .aether and .agents dirs
      vi.mocked(fs.readdir).mockReset();
      const projectQwenDir = path.join('/test/project', '.aether', 'skills');
      ...
      vi.mocked(fs.readFile).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.includes('.aether') && pathStr.includes('shared-skill')) {
          return Promise.resolve(
      ...
      // Only one instance should remain, from .aether (first in PROVIDER_CONFIG_DIRS)
      expect(skills).toHaveLength(1);
      expect(skills[0].name).toBe('shared-skill');
      expect(skills[0].description).toBe('From aether dir');
    });
  ...
  describe('getSkillsBaseDirs', () => {
    it('should return all project-level base dirs', () => {
      ...
      expect(baseDirs).toContain(path.join('/test/project', '.aether', 'skills'));
    ...
    it('should return all user-level base dirs', () => {
      ...
      expect(baseDirs).toHaveLength(2);
      expect(baseDirs).toContain(path.join('/home/user', '.aether', 'skills'));
  ...
  describe('bundled skills', () => {
    ...
    const projectDirSegment = path.join('.aether', 'skills');
    const userDirSegment = path.join('.aether', 'skills');
    ...
    function mockReaddirForLevels(levels: Set<string>) {
      vi.mocked(fs.readdir).mockImplementation((dirPath) => {
        const pathStr = String(dirPath);
        const isBundled =
          pathStr.endsWith(bundledDirSegment) && !pathStr.includes('.aether');
        const isProject =
