...
describe('SubagentManager', () => {
  ...
  const validConfig: SubagentConfig = {
    ...
    level: 'project',
    filePath: '/test/project/.aether/agents/test-agent.md',
  };
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.aether/agents/test-agent.md';
      const userPath = '/home/user/.aether/agents/test-agent.md';
...
`;
    ...
    it('should not warn when filename matches subagent name', () => {
      ...
      const matchingPath = '/test/project/.aether/agents/test-agent.md';
  ...
  describe('loadSubagent', () => {
    it('should load subagent from project level first', async () => {
      ...
      expect(fs.readFile).toHaveBeenCalledWith(
        path.normalize('/test/project/.aether/agents/test-agent.md'),
        'utf8',
      );
    });
    ...
    it('should fall back to user level if project level fails', async () => {
      ...
      expect(fs.readFile).toHaveBeenCalledWith(
        path.normalize('/home/user/.aether/agents/test-agent.md'),
        'utf8',
      );
    });
...
You are a helpful assistant.`;
      ...
      expect(fs.unlink).toHaveBeenCalledWith(
        path.normalize('/test/project/.aether/agents/wrong-name.md'),
      );
    });
...
Target content`,
      ...
      expect(fs.unlink).toHaveBeenCalledWith(
        path.normalize('/test/project/.aether/agents/target-file.md'),
      );
    });
  });
