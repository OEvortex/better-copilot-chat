...
export const GITHUB_WORKFLOW_PATHS = [
  'aether-dispatch/aether-dispatch.yml',
  'aether-assistant/aether-invoke.yml',
  'issue-triage/aether-triage.yml',
  'issue-triage/aether-scheduled-triage.yml',
  'pr-review/aether-review.yml',
];
...
// Add Aether Code specific entries to .gitignore file
export async function updateGitignore(gitRepoRoot: string): Promise<void> {
  const gitignoreEntries = ['.aether/', 'gha-creds-*.json'];
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/OEvortex/aether-action/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/OEvortex/aether-action/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
    ...
    return {
      ...
      toolArgs: {
        description:
          'Setting up GitHub Actions to triage issues and review PRs with Aether.',
        command,
        is_background: false,
      },
    };
