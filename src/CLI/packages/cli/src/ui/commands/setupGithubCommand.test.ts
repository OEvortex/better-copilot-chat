...
describe('setupGithubCommand', async () => {
  ...
  it('returns a tool action to download github workflows and handles paths', async () => {
    ...
    const expectedSubstrings = [
      `set -eEuo pipefail`,
      `fakeOpenCommand "https://github.com/OEvortex/aether-action`,
    ];
    ...
    if (gitignoreExists) {
      ...
      expect(gitignoreContent).toContain('.aether/');
...
describe('updateGitignore', () => {
  ...
  it('adds only missing entries when some already exist', async () => {
    ...
    // Should not duplicate .aether/ entry
    expect((content.match(/\.aether\//g) || []).length).toBe(1);
  });

  it('does not get confused by entries in comments or as substrings', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = [
      '# This is a comment mentioning .aether/ folder',
      'my-app.aether/config',
    ...
    expect(content).toContain('.aether/');
    expect(content).toContain('gha-creds-*.json');
    ...
    const lines = content
      ...
      .filter((line) => line);
    expect(lines).toContain('.aether/');
    expect(lines).toContain('gha-creds-*.json');
    expect(lines).toContain('my-app.aether/config');
