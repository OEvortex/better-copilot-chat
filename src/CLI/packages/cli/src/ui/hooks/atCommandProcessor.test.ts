...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectAetherIgnore: () => true,
  ...
  describe('aether-ignore filtering', () => {
    it('should skip aether-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.aetherignore'),
        'build/output.js',
      );
      const aetherIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${aetherIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${aetherIgnoredFile} is aether-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\naether-ignored: ${aetherIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .aetherignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.aetherignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed aether-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.aetherignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${aetherIgnoredFile}`;
    ...
    expect(result).toMatchObject({
      processedQuery: [
        { text: `@${validFile} @${aetherIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from ${validFile}:\n` },
