...
export async function getProviderLiveModels(provider: RegistryProvider): Promise<string[]> {
    const baseUrl = provider.profile
        ? provider.profile.env.OPENAI_BASE_URL ||
          provider.profile.env.GEMINI_BASE_URL ||
          provider.profile.env.ANTHROPIC_BASE_URL
    ...
    const apiKey = provider.profile
        ? provider.profile.env.OPENAI_API_KEY ||
          provider.profile.env.GEMINI_API_KEY ||
          provider.profile.env.ANTHROPIC_API_KEY ||
          provider.profile.env.CODEX_API_KEY
        : provider.apiKey;
...
export const AETHER_PROVIDER_SNAPSHOT_JSON_ENV = 'AETHER_PROVIDER_SNAPSHOT_JSON';
export const AETHER_PROVIDER_SNAPSHOT_FILE_ENV = 'AETHER_PROVIDER_SNAPSHOT_FILE';
...
function toSnapshotProvider(entry: SnapshotProviderEntry): RegistryProvider {
    ...
    const compatConfig: ProviderCompatConfig | undefined = baseUrl ? { baseUrl } : undefined;
...
function inferSdkMode(config: ProviderConfig): SdkMode | undefined {
    const modes = new Set(config.models.map(model => model.sdkMode).filter((mode): mode is SdkMode => Boolean(mode)));
...
function toConfigProvider(id: string, config: (typeof KnownProviders)[string]): RegistryProvider {
    ...
    const baseUrl =
        ...
        config.anthropic?.baseUrl?.trim() ||
        config.responses?.baseUrl?.trim() ||
        undefined;
...
export function parseProviderSnapshot(raw: string | undefined): ProviderSnapshot | null {
    if (!raw) {
        return null;
    }
    ...
    try {
        ...
        const providers = parsed.providers.filter((entry): entry is SnapshotProviderEntry =>
            Boolean(
                entry &&
                    typeof entry === 'object' &&
                    typeof entry.id === 'string' &&
                    (typeof entry.displayName === 'string' || typeof entry.label === 'string') &&
                    entry.profile &&
...
export function getProvidersFromSnapshot(snapshot: ProviderSnapshot): RegistryProvider[] {
...
export function getProvider(
    providerId: string,
    snapshot: ProviderSnapshot | null = parseProviderSnapshot(process.env[AETHER_PROVIDER_SNAPSHOT_JSON_ENV]) ||
