...
export function getProviderWizardDefaults(
  ...
  return {
    ...
    anthropicModel: safeAnthropicModel,
    geminiModel: safeGeminiModel,
  }
}

type RegistryProviderChoiceInfo = {
  label: string
  value: ProviderChoice
  description: string
}

function getRegistryProviderChoices(): RegistryProviderChoiceInfo[] {
  return getAllProviders()
    .filter(provider => provider.id !== 'compatible')
    .map(provider => ({
      label: provider.displayName,
      value: `registry:${provider.id}` as ProviderChoice,
      description:
...
function parseRegistryProviderChoice(
  ...
  return value.startsWith('registry:') ? value.slice('registry:'.length) : undefined
}

function resolveRegistryProviderSetup(providerId: string): {
  label: string
  profile: 'openai' | 'anthropic'
  baseUrl?: string
} | null {
  ...
  const profile =
    provider.sdkMode === 'anthropic' || provider.anthropic?.baseUrl
      ? 'anthropic'
      : 'openai'
  ...
  return {
    ...
    baseUrl:
      profile === 'anthropic'
        ? provider.anthropic?.baseUrl ?? provider.baseUrl
        : provider.openai?.baseUrl ?? provider.baseUrl,
  }
}
  ...
  switch (profile) {
    ...
    case 'anthropic':
      return {
        ...
        credentialLabel:
          maskSecretForDisplay(env.ANTHROPIC_API_KEY) !== undefined
            ? 'configured'
            : undefined,
      }
    case 'openai':
...
function ProviderChooser({
  ...
  const summary = buildCurrentProviderSummary()
  const registryOptions = getRegistryProviderChoices().map(
  ...
  const options: OptionWithDescription<ProviderChoice>[] = [
    {
      ...
      value: 'auto',
      description:
        'Prefer local Ollama when available, otherwise guide you into the best matching registry provider',
    },
    ...registryOptions,
  ]
          ...
          description={
            step.profile === 'anthropic'
              ? process.env.ANTHROPIC_API_KEY
                ? 'Enter an API key, or leave this blank to reuse the current ANTHROPIC_API_KEY from this session.'
                : 'Enter the API key for your Anthropic-compatible provider.'
        ...
        <TextEntryDialog
          ...
          onSubmit={value => {
            ...
            if (env) {
              finishProfileSave(
                onDone,
                step.profile === 'anthropic' ? 'anthropic' : 'openai',
                env,
              )
            }
          }}
