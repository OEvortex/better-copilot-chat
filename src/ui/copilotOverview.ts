...
export class CopilotOverview {
	...
	private static async sendStateUpdate(webview: vscode.Webview) {
		try {
			...
			const fimEnabled = vscode.workspace
				.getConfiguration("chp")
				.get<boolean>("fimCompletion.enabled", false);
			const nesEnabled = vscode.workspace
				.getConfiguration("chp")
				.get<boolean>("nesCompletion.enabled", false);
	...
	private static detachTokenListener() {
		if (CopilotOverview.tokenListener) {
			CopilotOverview.tokenListener.dispose();
			CopilotOverview.tokenListener = undefined;
		}
	}
}

export function registerCopilotOverviewCommand(
