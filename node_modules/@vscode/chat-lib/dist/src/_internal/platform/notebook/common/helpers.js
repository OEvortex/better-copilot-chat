...
function summarize(cell) {
    const cellType = cell.kind === vscodeTypesShim_1.NotebookCellKind.Code ? 'code' : 'markdown';
...
function notebookCellToCellData(cell) {
    const cellData = new vscodeTypesShim_1.NotebookCellData(cell.kind, cell.document.getText(), cell.document.languageId);
...
function getDefaultLanguage(notebook) {
    const codeCell = notebook.getCells().find(cell => cell.kind === vscodeTypesShim_1.NotebookCellKind.Code);
    if (codeCell) {
