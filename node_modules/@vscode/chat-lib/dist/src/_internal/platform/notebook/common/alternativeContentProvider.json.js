...
class AlternativeJsonDocument extends alternativeNotebookDocument_1.AlternativeNotebookDocument {
    fromCellPosition(cell, position) {
        ...
        const leadingCharacters = cell.document.getText(new vscodeTypesShim_1.Range(position.line, 0, position.line, position.character));
        ...
        // -1 to exclude to trailing `"`
        return new vscodeTypesShim_1.Position(linePositionInAltContent, characterPositionInAltContent.length);
    }
...
class AlternativeJsonNotebookContentProvider extends alternativeContentProvider_1.BaseAlternativeNotebookContentProvider {
    ...
    getAlternativeDocument(notebook, excludeMarkdownCells) {
        const cells = notebook.getCells().filter(cell => excludeMarkdownCells ? cell.kind !== vscodeTypesShim_1.NotebookCellKind.Markup : true).map(cell => {
    ...
    parseAlternateContentImpl(notebookOrUri, inputStream, token) {
        return new async_1.AsyncIterableObject(async (emitter) => {
            ...
            const cellInfo = {
                ...
                startOffset: -1,
                endOffset: -1,
                kind: vscodeTypesShim_1.NotebookCellKind.Code,
                source: [],
            };
            ...
            const emitCell = (endOffset) => {
                ...
                cellInfo.kind = cell?.kind || (cellInfo.language === 'markdown' ? vscodeTypesShim_1.NotebookCellKind.Markup : vscodeTypesShim_1.NotebookCellKind.Code);
                ...
                (0, jsonc_parser_1.visit)(jsonText, {
                    ...
                    onLiteralValue: (value, offset, _length, _startLine, _startCharacter, pathSupplier) => {
                        ...
                        if (cellInfo.index !== -1 && cellInfo.index !== cellIndex) {
                            ...
                            cellInfo.kind = vscodeTypesShim_1.NotebookCellKind.Code;
                            cellInfo.source = [];
                        ...
                        if (property === 'cell_type') {
                            cellInfo.kind = value === 'code' ? vscodeTypesShim_1.NotebookCellKind.Code : vscodeTypesShim_1.NotebookCellKind.Markup;
                            if (cellInfo.kind === vscodeTypesShim_1.NotebookCellKind.Markup) {
                            ...
                            if (cellInfo.language === 'markdown') {
                                cellInfo.kind = vscodeTypesShim_1.NotebookCellKind.Markup;
                            }
                        }
