...
class AlternativeTextDocument extends alternativeNotebookDocument_1.AlternativeNotebookDocument {
    ...
    fromCellPosition(cell, position) {
        ...
        const eolLength = cell.document.eol === vscodeTypesShim_1.EndOfLine.LF ? 1 : 2;
        ...
        const markdownOffset = cell.kind === vscodeTypesShim_1.NotebookCellKind.Markup ? blockComment[0].length + eolLength : 0;
...
class AlternativeTextNotebookContentProvider extends alternativeContentProvider_1.BaseAlternativeNotebookContentProvider {
    ...
    async *parseAlternateContent(notebookOrUri, inputStream, token) {
        ...
        for await (const lineOfText of inputStream) {
            ...
            if ((isLineCommentWithCellMarker || isLineCommentForEmptyCellWithoutCellMarker) && cellParts?.language) {
                ...
                const lineOfCellText = { index: -1, uri: undefined, language: undefined, kind: vscodeTypesShim_1.NotebookCellKind.Code, emitted: false, type: 'start' };
                ...
                lineOfCellText.kind = cell?.kind || (lineOfCellText.language === 'markdown' ? vscodeTypesShim_1.NotebookCellKind.Markup : vscodeTypesShim_1.NotebookCellKind.Code);
    ...
    getAlternativeDocumentFromText(text, notebook) {
        const blockComment = getBlockComment(notebook);
        const lineCommentStart = getLineCommentStart(notebook);
        const cellIdMap = (0, helpers_1.getCellIdMap)(notebook);
        const cellOffsetMap = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (isLineCommentWithCellMarker || isLineCommentForEmptyCellWithoutCellMarker) {
                ...
                if (cellParts) {
                    const cell = cellIdMap.get(cellParts.id) || notebook.getCells().find(c => c.document.languageId === cellParts.language &&
                    ...
                    if (cell) {
                        ...
                        const eolLength = helpers_1.EOL.length;
            ...
            currentOffset += line.length + helpers_1.EOL.length;
        }
        return new AlternativeTextDocument(text, cellOffsetMap, notebook);
    }
    getAlternativeDocument(notebook, excludeMarkdownCells) {
        const cells = notebook.getCells().filter(cell => excludeMarkdownCells ? cell.kind !== vscodeTypesShim_1.NotebookCellKind.Markup : true).map(cell => (0, helpers_1.summarize)(cell));
        const blockComment = getBlockComment(notebook);
        const lineCommentStart = getLineCommentStart(notebook);
