...
export declare abstract class BaseAlternativeNotebookContentProvider {
    readonly kind: 'xml' | 'text' | 'json';
    constructor(kind: 'xml' | 'text' | 'json');
    /**
     ...
     */
    abstract stripCellMarkers(text: string): string;
    /**
     * Generate the Document of the notebook document that is LLM friendly.
     */
    abstract getAlternativeDocument(notebook: NotebookDocument, excludeMarkdownCells?: boolean): AlternativeNotebookDocument;
    /**
     * Gets the alternative Document with specific text representation which
     * may have been model-edited.
     */
    abstract getAlternativeDocumentFromText(text: string, notebook: NotebookDocument): AlternativeNotebookDocument;
    /**
     ...
     */
    abstract getSummaryOfStructure(notebook: NotebookDocument, cellsToInclude: NotebookCell[], existingCodeMarker: string): string;
    /**
     ...
     */
    abstract parseAlternateContent(notebookOrUri: NotebookDocument | Uri, inputStream: AsyncIterable<LineOfText>, token: CancellationToken): AsyncIterable<LineOfCellText>;
}
//# sourceMappingURL=alternativeContentProvider.d.ts.map