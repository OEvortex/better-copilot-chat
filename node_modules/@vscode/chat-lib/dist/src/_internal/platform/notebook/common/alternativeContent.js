...
let AlternativeNotebookContentService = class AlternativeNotebookContentService {
    ...
    getFormat(options) {
        ...
        return this.configurationService.getExperimentBasedConfig(configurationService_1.ConfigKey.Advanced.NotebookAlternativeDocumentFormat, this.experimentationService);
    }
    create(format) {
...
function getAltNotebookRange(range, cellUri, notebook, format) {
    ...
    return new vscodeTypesShim_1.Range(doc.fromCellPosition(cell, range.start), doc.fromCellPosition(cell, range.end));
}
