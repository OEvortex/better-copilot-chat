...
class PositionOffsetTransformer {
    ...
    getPosition(offset) {
        ...
        return new vscodeTypesShim_1.Position(out.index, Math.min(out.remainder, lineLength));
    }
    toRange(offsetRange) {
        return new vscodeTypesShim_1.Range(this.getPosition(offsetRange.start), this.getPosition(offsetRange.endExclusive));
    }
    toOffsetRange(range) {
        return new offsetRange_1.OffsetRange(this.getOffset(range.start), this.getOffset(range.end));
    }
    toOffsetEdit(edits) {
        const validEdits = edits.map(edit => new vscodeTypesShim_1.TextEdit(this.validateRange(edit.range), edit.newText));
    ...
    toTextEdits(edit) {
        return edit.replacements.map(edit => {
            return new vscodeTypesShim_1.TextEdit(this.toRange(edit.replaceRange), edit.newText);
        });
    }
    validatePosition(position) {
        if (!(position instanceof vscodeTypesShim_1.Position)) {
        ...
        return new vscodeTypesShim_1.Position(line, character);
    }
    validateRange(range) {
        return new vscodeTypesShim_1.Range(this.validatePosition(range.start), this.validatePosition(range.end));
    }
}
