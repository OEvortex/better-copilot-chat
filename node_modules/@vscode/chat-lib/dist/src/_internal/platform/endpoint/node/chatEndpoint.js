...
let ChatEndpoint = ChatEndpoint_1 = class ChatEndpoint {
    constructor(modelMetadata, _domainService, _capiClientService, _fetcherService, _telemetryService, _authService, _chatMLFetcher, _tokenizerProvider, _instantiationService, _configurationService, _expService, _logService) {
    ...
    get urlOrRequestMetadata() {
        ...
        return this.modelMetadata.urlOrRequestMetadata ??
            (this.useResponsesApi ? { type: copilot_api_1.RequestType.ChatResponses } :
                this.useMessagesApi ? { type: copilot_api_1.RequestType.ChatMessages } : { type: copilot_api_1.RequestType.ChatCompletions });
    }
    get useResponsesApi() {
        if (this.modelMetadata.supported_endpoints
            && !this.modelMetadata.supported_endpoints.includes(endpointProvider_1.ModelSupportedEndpoint.ChatCompletions)
            && this.modelMetadata.supported_endpoints.includes(endpointProvider_1.ModelSupportedEndpoint.Responses)) {
            return true;
        }
        ...
        return !!(enableResponsesApi && this.modelMetadata.supported_endpoints?.includes(endpointProvider_1.ModelSupportedEndpoint.Responses));
    }
    get useMessagesApi() {
        const enableMessagesApi = this._configurationService.getExperimentBasedConfig(configurationService_1.ConfigKey.TeamInternal.UseMessagesApi, this._expService);
        return !!(enableMessagesApi && this.modelMetadata.supported_endpoints?.includes(endpointProvider_1.ModelSupportedEndpoint.Messages));
    }
    ...
    createRequestBody(options) {
        if (this.useResponsesApi) {
            const body = this._instantiationService.invokeFunction(responsesApi_1.createResponsesRequestBody, options, this.model, this);
            return this.customizeResponsesBody(body);
        }
        else if (this.useMessagesApi) {
            const body = this._instantiationService.invokeFunction(messagesApi_1.createMessagesRequestBody, options, this.model, this);
    ...
    customizeMessagesBody(body) {
        return body;
    }
    customizeResponsesBody(body) {
        return body;
    }
    customizeCapiBody(body, options) {
        const isConversationAgent = options.location === commonTypes_1.ChatLocation.Agent;
        if ((0, chatModelCapabilities_1.isAnthropicFamily)(this) && !options.disableThinking && isConversationAgent) {
            const configuredBudget = this._configurationService.getExperimentBasedConfig(configurationService_1.ConfigKey.AnthropicThinkingBudget, this._expService);
    ...
    async processResponseFromChatEndpoint(telemetryService, logService, response, expectedNumChoices, finishCallback, telemetryData, cancellationToken) {
        if (this.useResponsesApi) {
            ...
            return (0, messagesApi_1.processResponseFromMessagesEndpoint)(this._instantiationService, telemetryService, logService, response, expectedNumChoices, finishCallback, telemetryData);
        }
    ...
    cloneWithTokenOverride(modelMaxPromptTokens) {
        return this._instantiationService.createInstance(ChatEndpoint_1, (0, objects_1.mixin)((0, objects_1.deepClone)(this.modelMetadata), { capabilities: { limits: { max_prompt_tokens: modelMaxPromptTokens } } }));
    }
};
