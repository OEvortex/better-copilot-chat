...
class LlmNESTelemetryBuilder extends lifecycle_1.Disposable {
    build(includeAlternativeAction) {
        ...
        if (this._request) {
            ...
            if (git) {
                const activeDocRepository = git.getRepository(vscodeTypesShim_1.Uri.parse(activeDoc.id.uri));
                ...
                const repositories = [...new Set(this._request.documents.map(doc => git.getRepository(vscodeTypesShim_1.Uri.parse(doc.id.uri))).filter(Boolean))];
