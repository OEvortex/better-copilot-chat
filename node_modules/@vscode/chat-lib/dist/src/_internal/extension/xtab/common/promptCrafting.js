...
exports.countTokensForLines = countTokensForLines;
...
exports.constructTaggedFile = constructTaggedFile;
...
class PromptPieces {
    constructor(currentDocument, editWindowLinesRange, areaAroundEditWindowLinesRange, activeDoc, xtabHistory, taggedCurrentDocLines, areaAroundCodeToEdit, langCtx, aggressivenessLevel, computeTokens, opts) {
        ...
        this.taggedCurrentDocLines = taggedCurrentDocLines;
...
function addRelatedInformation(relatedInformation, prompt, position) {
    if (position === 'before') {
        return appendWithNewLineIfNeeded(relatedInformation, prompt, 2);
    }
    return appendWithNewLineIfNeeded(prompt, relatedInformation, 2);
}
function appendWithNewLineIfNeeded(base, toAppend, minNewLines) {
...
function getPostScript(strategy, currentFilePath, aggressivenessLevel) {
    let postScript;
    switch (strategy) {
        ...
        case xtabPromptOptions_1.PromptingStrategy.Xtab275:
            ...
            break;
        case xtabPromptOptions_1.PromptingStrategy.XtabAggressiveness:
            postScript = `<|aggressive|>${aggressivenessLevel}<|/aggressive|>`;
            break;
        case xtabPromptOptions_1.PromptingStrategy.SimplifiedSystemPrompt:
        case xtabPromptOptions_1.PromptingStrategy.CopilotNesXtab:
        case undefined:
...
function createTaggedCurrentFileContentUsingPagedClipping(currentDocLines, areaAroundCodeToEdit, areaAroundEditWindowLinesRange, computeTokens, pageSize, opts) {
    ...
    return result_1.Result.ok(taggedCurrentFileContent);
}
function constructTaggedFile(currentDocument, editWindowLinesRange, areaAroundEditWindowLinesRange, promptOptions, computeTokens, opts) {
    ...
    const taggedCurrentFileContentResult = createTaggedCurrentFileContentUsingPagedClipping(currentFileContentLines, areaAroundCodeToEditForCurrentFile, areaAroundEditWindowLinesRange, computeTokens, promptOptions.pagedClipping.pageSize, promptOptions.currentFile);
